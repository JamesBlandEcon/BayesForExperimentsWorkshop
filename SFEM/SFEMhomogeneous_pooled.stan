...
parameters {
  ...
  real<lower=0.0,upper=0.5> gamma; // mean tremble probability (transformed)
...
model {
  ...
  for (ii in 1:N) {
      ...
      row_vector[S] like_strg = to_row_vector(y[ii,]).*rep_row_vector(log(1.0-gamma),S)
                              +to_row_vector(n[ii]-y[ii,]).*rep_row_vector(log(gamma),S)
